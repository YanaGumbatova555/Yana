...
//	$smarty->caching      = true; 
	...
	if(isset($_GET["hurl"]))
	{
		...
		$q = db_query("SELECT * FROM ".PRODUCTS_TABLE." where hurl='".$_GET["hurl"]."'") or die (db_error());
		$row = db_fetch_row($q);
		if($row) 
			{$productID = $row[0];}
		else	{
			$q = db_query("SELECT * FROM ".CATEGORIES_TABLE." where hurl='".$_GET["hurl"]."'") or die (db_error());
			$row = db_fetch_row($q);
			if($row) $_POST["categoryID"] = $row[0];
			}
	}

	if(isset($_GET["thurl"]))
	{
		$q = db_query("SELECT * FROM ".TAGS_TABLE." where hurl='".$_GET["thurl"]."'") or die (db_error());
		$row = db_fetch_row($q);
